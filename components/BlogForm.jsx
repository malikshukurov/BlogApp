...
useEffect(() => {
  ...
  const handleSubmit = (e) => {
    ...
    // Yuxarıdan gələn funksiyanı çağırırıq
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-8">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div>
          <label className="block text-sm font-medium text-slate-700 mb-2">Başlıq (Title)</label>
          <input
            type="text"
            required
        ...
        <div>
          <label className="block text-sm font-medium text-slate-700 mb-2">Məzmun (Body)</label>
          <textarea
            required
            rows="6"
            className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white transition-all text-slate-900"
